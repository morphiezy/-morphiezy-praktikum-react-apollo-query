...
import { GET_TODO_LIST, GET_TODO_BY_PARAM } from "./GraphQL/TodoList/queries";
...
const TodoList = () => {
  ...
  ] = useLazyQuery(GET_TODO_BY_PARAM);
  ...
  /* Feature : Do Query when user input & click find button */

  const findTodoByTitle = () => {
    if (select === "" || title === "") return false;

    setInitialQuery(false);
    getTodo({ variables: { user_id: select, title: `%${title}%` } });
    setTitle("");
    setSelect("");
  };
    ...
    <>
      <div className="container">
        ...
        {loading || lazyLoading ? (
          ...
          <>
            {lazyData.todolist.length === 0 ? (
              <div className="empty-state">
                <svg className="checklist-icon">
                  <use href="#checklist-icon"></use>
                </svg>
                <h2 className="empty-state__title">Opss todo not found</h2>
                <p className="empty-state__description">
                  Make sure you input correctly todo that's user have
                </p>
              </div>
            ) : (
              <ul className="todo-list js-todo-list">
                {lazyData.todolist.map((todo) => (
                  <Todo
                    key={todo.id}
                    id={todo.id}
                    title={todo.title}
        ...
        {!(loading || lazyLoading) ? (
          <form className="js-form" onSubmit={(e) => e.preventDefault()}>
            <div className="user-input-area">
              <input
                ...
                placeholder="Input new todo or search existing todo by title"
            ...
            <input type="button" value="Find Todo" onClick={findTodoByTitle} />
          </form>
        ) : (
          false
        )}
